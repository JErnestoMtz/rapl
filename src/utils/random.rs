...
impl NdarrRand {
    ...
    pub fn rand_f32<R: Unsigned, D: Into<Dim<R>>>(shape: D) -> Ndarr<f32, R> {
        ...
        let random_vector: Vec<f32> = (0..n).map(|_| rng.sample(Uniform::new(0.0, 1.0))).collect();
    ...
    pub fn rand_f64<R: Unsigned, D: Into<Dim<R>>>(shape: D) -> Ndarr<f64, R> {
        ...
        let n = d.get_number_elements();
        let mut rng = rand::thread_rng();
        let random_vector: Vec<f64> = (0..n).map(|_| rng.sample(Uniform::new(0.0, 1.0))).collect();
        Ndarr::new(&random_vector, d).unwrap()
    }
    pub fn uniform<T: SampleUniform + Debug + Clone + Default, R: Unsigned, D: Into<Dim<R>>>(
        range: (T, T),
        shape: D,
    ) -> Ndarr<T, R> {
        let d: Dim<R> = shape.into();
