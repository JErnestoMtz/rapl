...
use num_traits::{Float, Unsigned, Pow};
use std::fmt::Debug;
...
impl<T: Debug + Clone + Add<Output = T> + Div<Output = T>+ Sub<Output = T>> Ndarr<T,U1> {
    ...
    {
         let segments: T = (n - 1)
        .try_into().expect("n too large, max value 2^16");
        ...
        T: Pow<T, Output = T> + Mul<Output = T> + Copy
    {
        ...
        let data: Vec<T> = (0..n).map(|i| base.pow(start + step*i.try_into().expect("n too large"))).collect();
        Ndarr::new(&data, [data.len()]).unwrap()
        ...
        T: Pow<T, Output = T> + Mul<Output = T> + Copy
    {
        ...
        let data: Vec<T> = (0..n).map(|i| start * ratio.pow(i.try_into().expect("n too large"))).collect();
        Ndarr::new(&data, [data.len()]).unwrap()
...
mod mesh_test{
    ...
    fn logspace(){
        ...
        assert!(x.approx(&Ndarr::from([1.,1e1, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9])));
    }
