...
impl Ndarr<u8,3>{
    ...
    pub fn save_as_rgb(&self, path: &dyn AsRef<Path>, fmt: ImageFormat) {
            assert!(self.shape[2] == 3, "Cannot convert Ndarr of shape {:?} to RGB image", self.shape);    
            let w = self.shape[0];
            let h = self.shape[1];
            let img: ImageBuffer<Rgb<u8>, Vec<u8>> = ImageBuffer::from_raw(w as u32, h as u32, self.data.clone()).expect("Could not create ImageBuffer with Ndarr");
            img.save_with_format(path, fmt).expect("Error, saving array as Image");
    }
}
...
impl  Ndarr<f32,3>{
    ...
    pub fn save_as_rgb(&self, path: &dyn AsRef<Path>, fmt: ImageFormat) {
            ...
            let w = self.shape[0];
            let h = self.shape[1];
            let img: ImageBuffer<Rgb<f32>, Vec<f32>> = ImageBuffer::from_raw(w as u32, h as u32, self.data.clone()).expect("Could not create ImageBuffer with Ndarr");
            img.save_with_format(path, fmt).expect("Error, saving array as Image");
    }
}
...
impl Ndarr<u8,2>{
    ...
    pub fn save_as_luma(&self, path: &dyn AsRef<Path>, fmt: ImageFormat) {
            let w = self.shape[0];
            let h = self.shape[1];
            let img: ImageBuffer<Luma<u8>, Vec<u8>> = ImageBuffer::from_raw(w as u32, h as u32, self.data.clone()).expect("Could not create ImageBuffer with Ndarr");
            img.save_with_format(path, fmt).expect("Error, saving array as Image");
    }
}
...
impl Ndarr<f32,2>{
    ...
    pub fn save_as_luma(&self, path: &dyn AsRef<Path>, fmt: ImageFormat) {
            let max = self.data.iter().max_by(|a, b| a.total_cmp(b)).expect("Cannot convert empty Ndarr to Image");
            let min = self.data.iter().min_by(|a, b| a.total_cmp(b)).expect("Cannot convert empty Ndarr to Image");

            let norm_arr = (self - min) / (max - min);
            let w = self.shape[0];
            let h = self.shape[1];
            let im_u8 = norm_arr.map_types(|x| (*x * u16::MAX as f32) as u16);
            let img: ImageBuffer<Luma<u16>, Vec<u16>> = ImageBuffer::from_raw(w as u32, h as u32, im_u8.data).expect("Could not create ImageBuffer with Ndarr");
            img.save_with_format(path, fmt).expect("Error, saving array as Image");
    }
}
