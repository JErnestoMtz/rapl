...
pub fn poly_diatic<F, T1, T2, T3, const R1: usize, const R2: usize>(
    arr1: &Ndarr<T1, R1>,
    arr2: &Ndarr<T2, R2>,
    f: F,
) -> Result<Ndarr<T3, { const_max(R1, R2) }>, DimError>
where
...
pub fn mat_mul<T, const R1: usize, const R2: usize>(
    arr1: Ndarr<T, R1>,
    arr2: Ndarr<T, R2>,
) -> Ndarr<T, { const_max(R1 + R2 - 1, R1 + R2 - 1) - 1 }>
where
...
pub fn inner_product<F, G, T1, T2, T3, const R1: usize, const R2: usize>(
    f: F,
    g: G,
    arr1: Ndarr<T1, R1>,
    arr2: Ndarr<T2, R2>,
) -> Ndarr<T3, { const_max(R1 + R2 - 1, R1 + R2 - 1) - 1 }>
where
    T1: Clone + Debug + Default,
    T2: Clone + Debug + Default,
    T3: Clone + Debug + Default,
...
pub fn inner_closure<F, G, T1, T2, T3, const R1: usize, const R2: usize>(
    f: F,
    g: G,
) -> impl Fn(Ndarr<T1, R1>, Ndarr<T2, R2>) -> Ndarr<T3, { const_max(R1 + R2 - 1, R1 + R2 - 1) - 1 }>
where
    T1: Clone + Debug + Default,
    T2: Clone + Debug + Default,
    T3: Clone + Debug + Default,
...
pub fn outer_product<F, T1, T2, T3, const R1: usize, const R2: usize>(
    f: F,
    arr1: Ndarr<T1, R1>,
    arr2: Ndarr<T2, R2>,
) -> Ndarr<T3, { const_max(R1 + R2, R1 + R2) }>
where
    T1: Clone + Debug + Default,
    T2: Clone + Debug + Default,
    T3: Clone + Debug + Default,
